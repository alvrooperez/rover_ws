...
class BrazoNode(Node):
    def __init__(self):
        ...
        self.posiciones_actuales = {4: 110, 5: 0, 6: 0, 7: 85, 11: 100}
...
def main(args=None):
    ...
    # --- NUEVA SECUENCIA ACTUALIZADA ---
    # Regla: Si un motor no aparece en el paso, mantiene su valor anterior.
    pasos = [
        {
            "nombre": "1. POSICIÓN INICIAL", 
            "angulos": {4: 110, 5: 0, 6: 0, 7: 85, 11: 100}, 
            "base": 0.0
        },
        {
            "nombre": "2. AJUSTE MOTOR 4", 
            "angulos": {4: 80}, 
            "base": 0.0
        },
        {
            "nombre": "3. MOVIMIENTO 6 Y 11", 
            "angulos": {6: 70, 11: 160}, 
            "base": 0.0
        },
        {
            "nombre": "4. RETROCESO MOTOR 6", 
            "angulos": {6: 0}, 
            "base": 0.0
        },
        {
            "nombre": "5. MOVIMIENTO COMBINADO (4, 6, 11)", 
            "angulos": {4: 50, 6: 55, 11: 100}, 
            "base": 0.0
        },
        {
            "nombre": "6. LIMPIEZA MOTOR 6", 
            "angulos": {6: 0}, 
            "base": 0.0
        },
        {
            "nombre": "7. FINALIZACIÓN MOTOR 4", 
            "angulos": {4: 100}, 
            "base": 0.0
        }
    ]
    ...
    try:
        for paso in pasos:
            ...
            # Movimiento de Servos: Solo se mueven los indicados en el paso
            for motor in orden_servos:
                if motor in paso['angulos']:
                    objetivo = paso['angulos'][motor]
                    node.mover_suave(motor, objetivo, velocidad=0.5)
                    # El método mover_suave ya actualiza self.posiciones_actuales[motor]
                    time.sleep(0.05)
        ...
        node.get_logger().info("Secuencia interrumpida por el usuario.")
    
    node.get_logger().info("Secuencia completada.")
    node.destroy_node()
    rclpy.shutdown()
